...
// =====================================================================
// ИЗМЕНЁННЫЙ МЕТОД: добавлен параметр pcnt_unit
// pcnt_unit = -1  →  автовыбор (оригинальное поведение)
// pcnt_unit = 0..7 →  использовать конкретный PCNT-юнит
// =====================================================================
void ESP32Encoder::attach(int a, int b, encType et, int pcnt_unit) {
	if (attached) {
		ESP_LOGE(TAG_ENCODER, "attach: already attached");
		return;
	}
	int index = 0;
	if (pcnt_unit >= 0 && pcnt_unit < MAX_ESP32_ENCODERS) {
		// --- Явный выбор юнита ---
		if (ESP32Encoder::encoders[pcnt_unit] != NULL) {
			ESP_LOGE(TAG_ENCODER, "PCNT unit %d already in use, FAIL!", pcnt_unit);
			return;
		}
		index = pcnt_unit;
		encoders[index] = this;
	} else {
		// --- Автовыбор: первый свободный слот (оригинальное поведение) ---
...
void ESP32Encoder::attachHalfQuad(int aPintNumber, int bPinNumber, int pcnt_unit) {
	attach(aPintNumber, bPinNumber, encType::half, pcnt_unit);

}
void ESP32Encoder::attachSingleEdge(int aPintNumber, int bPinNumber, int pcnt_unit) {
	attach(aPintNumber, bPinNumber, encType::single, pcnt_unit);
}
void ESP32Encoder::attachFullQuad(int aPintNumber, int bPinNumber, int pcnt_unit) {
	attach(aPintNumber, bPinNumber, encType::full, pcnt_unit);
}
