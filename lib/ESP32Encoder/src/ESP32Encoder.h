...
public:
	...
	~ESP32Encoder();
	void attachHalfQuad(int aPintNumber, int bPinNumber, int pcnt_unit = -1);
	void attachFullQuad(int aPintNumber, int bPinNumber, int pcnt_unit = -1);
	void attachSingleEdge(int aPintNumber, int bPinNumber, int pcnt_unit = -1);
	int64_t getCount();
	int64_t clearCount();
	int64_t pauseCount();
	int64_t resumeCount();
...
private:
	...
	void attach(int aPintNumber, int bPinNumber, encType et, int pcnt_unit = -1);
	int64_t getCountRaw();
	bool attached;
  bool direction;
  bool working;
