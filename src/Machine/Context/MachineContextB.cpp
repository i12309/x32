...
void addIssue(std::vector<String>& issues, const char* name, bool missing) {
    if (!missing) return;
    issues.push_back(String("[MachineContextB] Missing required object: ") + name);
}

} // namespace

MachineContextB& MachineContextB::instance() {
    static MachineContextB ctx;
    return ctx;
}

void MachineContextB::bind(Registry& registry) {
...
bool MachineContextB::readyForMotion() const {
...
void MachineContextB::collectIssues(std::vector<String>& issues) const {
