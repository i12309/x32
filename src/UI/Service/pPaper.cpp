...
void pPaper::pop_bDetectPaper(void* ptr) {
    ...
    // Removed Scene method call: paperStop(Catalog::StopMode::ForceStop).
    (void)UI;
    // Removed Scene method call: paperDetectPaper(UI.throw_switch, UI.detectDir, Catalog::SPEED::Custom, UI.clutch_switch, 0, UI.opticalSensor).
}
...
void pPaper::pop_bDetectMark(void* ptr) {
    ...
    // Removed Scene method call: paperStop(Catalog::StopMode::ForceStop).
    (void)UI;
    // Removed Scene method call: paperDetectMark(UI.throw_switch, UI.detectDir, Catalog::SPEED::Custom, UI.clutch_switch, 0, UI.opticalSensor).
}
...
void pPaper::push_bPForward(void* ptr) {
    ...
    // Removed Scene method call: paperMove(v, Catalog::DIR::Forward, Catalog::SPEED::Slow, true, UI.clutch_switch, UI.throw_switch, useEncoderCorrection).
}
...
void pPaper::pop_bPForward(void* ptr) {
    ...
    pPaper& UI = pPaper::getInstance();
    (void)UI;
    // Removed Scene method call: paperStop(Catalog::StopMode::SoftStop).
}
...
void pPaper::push_bPBack(void* ptr) {
    ...
    // Removed Scene method call: paperMove(v, Catalog::DIR::Backward, Catalog::SPEED::Slow, true, UI.clutch_switch, UI.throw_switch, useEncoderCorrection).
}
...
void pPaper::pop_cCheck(void* ptr) {
    Log::D(__func__);
    // Removed Scene method call: guillotineWork(Catalog::DIR::Forward, 350, Catalog::SPEED::Normal).
}
